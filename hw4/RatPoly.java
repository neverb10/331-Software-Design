...
public final class RatPoly {
	...
	private List<RatTerm> duplicate() {
		List<RatTerm> dup = new ArrayList<RatTerm>();
	...
	private static void scaleCoeff(List<RatTerm> lst, RatNum scalar) {
		for (int i = 0; i < lst.size(); i++) {
			RatNum c = lst.get(i).getCoeff().mul(scalar);
			if (!c.equals(0)) {
	...
	private static void incremExpt(List<RatTerm> lst, int degree) {
		for (int i = 0; i < lst.size(); i++) {
			int d = lst.get(i).getExpt() + degree;
			if (d >= 0) {
				lst.set(i, new RatTerm(lst.get(i).getCoeff(), d));
			}
		}
	}
	...
	private static void sortedInsert(List<RatTerm> lst, RatTerm newTerm) {
		...
		for (RatTerm rt : lst) {
			...
			if (newTerm.getExpt() > rt.getExpt()) {
				...
				RatNum c = newTerm.getCoeff().add(rt.getCoeff());
				lst.set(index, new RatTerm(c, rt.getExpt()));
	...
	public RatPoly negate() {		
		...
		List<RatTerm> dup = duplicate();
		for (int i = 0; i < dup.size(); i++) {
			dup.set(i, new RatTerm(dup.get(i).getCoeff().mul(new RatNum(-1)), dup.get(i).getExpt()));
		}
		return new RatPoly(dup);
	}
	...
	public RatPoly mul(RatPoly p) {
		...
		for (int i = 0; i < terms.size(); i++) {
			List<RatTerm> dup = p.duplicate();
			scaleCoeff(dup, terms.get(i).getCoeff());
			incremExpt(dup, terms.get(i).getExpt());
			result.add(new RatPoly(dup));
		}
		return result;
	}
	...
	public RatPoly div(RatPoly p) {
		...
		//constantly modified dividend
		RatPoly intermediate = new RatPoly(this.duplicate());
		//while the intermediate dividend's first term's exponent is less than or equal to p's
		//(in which case there would be a remainder) and intermediate is not 0 (we would be done)
		while (intermediate.degree() >= p.degree() && intermediate.terms.size() != 0) {
			RatPoly partialResult = new RatPoly(intermediate.terms.get(0).div(p.terms.get(0)));
			intermediate = intermediate.sub(p.mul(partialResult));
			quotient.add(partialResult);
		}
		quotient.checkRep();
		return quotient;
	}
	...
	public RatPoly differentiate() {
		...
		for (int i = 0; i < terms.size(); i++) {
			if (terms.get(i).getExpt() != 0) {
				derivative.terms.add(terms.get(i).differentiate());
			}
		}
		return derivative;
	}
	...
	public RatPoly antiDifferentiate(RatNum integrationConstant) {
		...
		RatPoly antiderivative = new RatPoly();
		for (int i = 0; i < terms.size(); i++) {
			antiderivative.terms.add(terms.get(i).antiDifferentiate());

		}
		if (!integrationConstant.equals(RatNum.ZERO)) {
			antiderivative.terms.add(new RatTerm(integrationConstant, 0));
		}
